...
public static class Utility
{
    ...
    /// <summary>
    /// Returns a random rotation around one axis
    /// </summary>
    /// <returns></returns>
    public static Quaternion ReturnRandomRotation(Vector3 rotationAxis)
    {
        Quaternion quaternion = new Quaternion();
        quaternion = Quaternion.AngleAxis(Random.Range(0, 360), rotationAxis);
        return quaternion;
    }
    ...
    public static Transform GetClosest(List<Transform> list, Transform origin)
    {
        float minDistance = float.MaxValue;
        Transform closest = default;
        foreach (var item in list)
        {
            float distance = Vector3.Distance(item.position, origin.position);
            if (distance < minDistance)
            {
                minDistance = distance;
                closest = item;
    ...
    public static void RemoveAllChildrenEditor(Transform parent)
    {
    ...
    public static string AddTextOnNewLine(string originalText, string textToAdd)
    {
        if (textToAdd == "")
            return originalText;
        if (originalText != "")
